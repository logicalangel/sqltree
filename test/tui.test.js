...
describe('startTui', () => {
  ...
  it('registers up/down/left/right/tab/e/s/r/d/w keys', async () => {
    await startTui(mockAdapter);
    ...
    expect(keyHandlers['w']).toBeDefined();
    expect(keyHandlers['s']).toBeDefined();
  });
  ...
  it('w/s keys scroll detail', async () => {
    await startTui(mockAdapter);
    keyHandlers['w']();
    keyHandlers['s']();
    expect(mockScreen.render).toHaveBeenCalled();
